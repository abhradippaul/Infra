...
resource "aws_api_gateway_integration_response" "tasks_options_integration_response" {
  ...
  resource_id = aws_api_gateway_resource.tasks.id
  http_method = aws_api_gateway_method.tasks_options.http_method
  status_code = aws_api_gateway_method_response.tasks_options_response.status_code
...
resource "aws_api_gateway_deployment" "example" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      aws_api_gateway_method.create_task_method.id,
...
resource "aws_lambda_permission" "allow_apigw_invoke_all" {
  for_each = {
    create = var.lambda_function_name_create_task
    get    = var.lambda_function_name_delete_task
    update = var.lambda_function_name_get_tasks
    delete = var.lambda_function_name_update_task
  }
